...
Once again, a **sample space** can be thought of as the set of all possible outcomes. Furthermore, the sample space can be tiny or infinite in size. For example, the set of all outcomes in a coin toss is 2, so the sample space $$\Omega =$$ {heads, tails}; the sample space, or set of all outcomes, of a die is 6 given 6 unique faces, is $$\Omega=$$ {1,2,3,4,5,6}. However, as an extreme example, the set of all outcomes that atoms can be arranged in the universe can be truly infinite (ever expanding infinite).
...
2. $$\sum P(s) = 1$$, where $$s \in \mathcal{F}$$
